...
class Transform {
    ...
    getNorm() {
        ...
        let m = this._matrix;
        let maxRowSum = abs(m[0]) + abs(m[1]) + abs(m[2]);
        let rowSum = abs(m[4]) + abs(m[5]) + abs(m[6]);

        if (rowSum > maxRowSum) {
            maxRowSum = rowSum;
        }
        rowSum = abs(m[8]) + abs(m[9]) + abs(m[10]);
        if (rowSum > maxRowSum) {
            maxRowSum = rowSum;
        }
    ...
    inverse() {
        ...
        im[3] = -(im[0] * t0 + im[1] * t1 + im[2] * t2);
        im[7] = -(im[4] * t0 + im[5] * t1 + im[6] * t2);
        im[11] = -(im[8] * t0 + im[9] * t1 + im[10] * t2);
    ...
    _updateMatrix() {
        if (this._isIdentity) {
            ...
            mm[3] = this._translate[0];
            mm[7] = this._translate[1];
            mm[11] = this._translate[2];
    ...
    static invert3x3(mat, invMat) {
        ...
        invMat[0] = mat[5] * mat[10] - mat[6] * mat[9];
        invMat[1] = mat[2] * mat[9] - mat[1] * mat[10];
        invMat[2] = mat[1] * mat[6] - mat[2] * mat[5];
        invMat[4] = mat[6] * mat[8] - mat[4] * mat[10];
        invMat[5] = mat[0] * mat[10] - mat[2] * mat[8];
        invMat[6] = mat[2] * mat[4] - mat[0] * mat[6];
        invMat[8] = mat[4] * mat[9] - mat[5] * mat[8];
        invMat[9] = mat[1] * mat[8] - mat[0] * mat[9];
        invMat[10] = mat[0] * mat[5] - mat[1] * mat[4];

        // Compute the reciprocal of the determinant of M.
        let invDet = 1 / (mat[0] * invMat[0] + mat[1] * invMat[4] + mat[2] * invMat[8]);
